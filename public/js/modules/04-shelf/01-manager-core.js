  ...
  function currentItems() {
    if (hasAnyPlatformLogin() && (userPlaylists.length || myPodcastCollections.length)) {
      ...
      var items = source.map(function (pl) {
        var provider = pl.provider === 'qq' ? 'qq' : (pl.provider === 'kw' ? 'kw' : (pl.provider === 'kugou' ? 'kugou' : (pl.provider === 'qishui' ? 'qishui' : (pl.provider === 'spotify' ? 'spotify' : 'netease'))));
        var sourceLabel = provider === 'qq' ? 'QQ' : (provider === 'kw' ? 'KW' : (provider === 'kugou' ? 'KG' : (provider === 'qishui' ? 'QS' : (provider === 'spotify' ? 'SP' : 'NE'))));
        ...
        return {
          ...
          cover: pl.cover || '', tag: (pl.shelfPane || pl.shelf_pane) === 'fav' || (!(pl.shelfPane || pl.shelf_pane) && pl.subscribed) ? '收藏歌单' : (provider === 'kw' ? '酷我歌单' : (provider === 'qishui' ? '汽水歌单' : '我的歌单')), playlistId: (provider === 'qq' ? 'qq:' : (provider === 'kw' ? 'kw:' : (provider === 'kugou' ? 'kugou:' : (provider === 'qishui' ? 'qishui:' : '')))) + pl.id, provider: provider
        };
      });
