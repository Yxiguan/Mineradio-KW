...
function playlistCatalogProviderArray(provider) {
  ...
  if (provider === 'kw') return kwPlaylists;
...
function setPlaylistCatalogProviderArray(provider, rows) {
  ...
  else if (provider === 'kw') kwPlaylists = rows;
...
function playlistCatalogProviderLoggedIn(provider) {
  ...
  if (provider === 'kw') return !!kuwoLoginStatus.loggedIn;
...
function playlistCatalogPageUrl(provider, offset, limit) {
  ...
  if (provider === 'kw') return '/api/kw/user/playlists';
...
function rebuildUserPlaylistsFromCatalog(opts) {
  opts = opts || {};
  userPlaylists = neteasePlaylists.concat(qqPlaylists, kugouPlaylists, kwPlaylists, qishuiPlaylists, spotifyPlaylists);
...
function requestNextPlaylistCatalogPage(reason) {
  ...
  var order = ['netease', 'spotify', 'qq', 'kugou', 'kw', 'qishui'];
...
async function refreshUserPlaylists(force) {
  if (!loginStatus.loggedIn && !qqLoginStatus.loggedIn && !kugouLoginStatus.loggedIn && !kuwoLoginStatus.loggedIn && !qishuiLoginStatus.loggedIn && !spotifyLoginStatus.loggedIn) {
  ...
  var catalogNeedsNewProvider = playlistCatalogSyncState.loading && ['netease', 'qq', 'kugou', 'kw', 'qishui', 'spotify'].some(function (provider) {
  ...
  ['netease', 'qq', 'kugou', 'kw', 'qishui', 'spotify'].forEach(function (provider) {
